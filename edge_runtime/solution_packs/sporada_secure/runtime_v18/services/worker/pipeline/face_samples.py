...
def _encode_jpeg(frame, quality: int = 88) -> bytes:
    ok, encoded = cv2.imencode(".jpg", frame, [int(cv2.IMWRITE_JPEG_QUALITY), quality])
    if not ok:
        raise RuntimeError("could not encode face evidence")
    return encoded.tobytes()


def _write_jpeg_bytes(path: Path, data: bytes) -> tuple[str, int]:
    path.parent.mkdir(parents=True, exist_ok=True)
...
class FaceSamplePipeline:
    def __init__(self, camera_id: str, edge_id: str, extractor, camera_config: dict) -> None:
        ...
        self.preferred_quality = max(self.min_quality, float(emission.get(
            "preferred_quality", os.getenv("FACE_PREFERRED_QUALITY", "0.65")
        )))
        self.selection_window = max(0.0, float(emission.get(
            "selection_window_seconds", os.getenv("FACE_SELECTION_WINDOW_SECONDS", "1.5")
        )))
    ...
    def process(self, packet, people: list) -> None:
        ...
        for face in faces:
            ...
            if not self._should_emit(face.track_id, embedding, now):
                ...
                self.pending_candidates.pop(face.track_id, None)
                continue
            current = self.pending_candidates.get(face.track_id)
            if current is None or float(face.quality) > current["quality"]:
                try:
                    self.pending_candidates[face.track_id] = self._capture_candidate(
                        packet, face, embedding, now,
                        current["deadline"] if current is not None else now + self.selection_window,
                    )
                except Exception:  # noqa: BLE001
                    self.metrics.snapshot_failure("event_frame_or_face_crop")
                    logger.exception("could not capture face candidate for camera=%s", self.camera_id)
                    continue
            candidate = self.pending_candidates[face.track_id]
            if candidate["quality"] >= self.preferred_quality:
                self.pending_candidates.pop(face.track_id, None)
                self._emit_candidate(packet, candidate, now)

        for track_id, candidate in list(self.pending_candidates.items()):
            if now < candidate["deadline"]:
                continue
            self.pending_candidates.pop(track_id, None)
            if self._should_emit(track_id, candidate["embedding"], now):
                self._emit_candidate(packet, candidate, now)
            else:
                self.metrics.suppressed("duplicate")
    ...
    def _refresh_policy(self) -> None:
        ...
        self.preferred_quality = max(self.min_quality, float(
            emission.get("preferred_quality", self.preferred_quality)
        ))
        self.selection_window = max(0.0, float(
            emission.get("selection_window_seconds", self.selection_window)
        ))
    ...
    def _capture_candidate(self, packet, face, embedding, now: float, deadline: float) -> dict[str, Any]:
        ...
        return {
            ...
            "frame_shape": tuple(packet.frame.shape),
            "observed_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(now)),
            "deadline": float(deadline),
            "event_frame_jpeg": _encode_jpeg(annotated),
            "face_crop_jpeg": _encode_jpeg(self._evidence_crop(packet.frame, face.bbox)),
        }

    def _emit_candidate(self, packet, candidate: dict[str, Any], now: float) -> None:
        ...
        try:
            assets = self._save_candidate_assets(candidate, sample_id)
        except Exception:  # noqa: BLE001
            self.metrics.snapshot_failure("event_frame_or_face_crop")
            logger.exception("could not persist selected face evidence for camera=%s", self.camera_id)
            return
        ...
        packet.add_event({
            ...
            "model_id": self.extractor.model_id,
            "face_quality": round(candidate["quality"], 6),
            "subject": {
                "type": "face",
                "track_id": candidate["track_id"],
                "confidence": round(candidate["detector_confidence"], 6),
                "bbox": self._normalized_bbox(candidate["bbox"], candidate["frame_shape"]),
            },
            "snapshot": self._event_asset(assets["event_frame"]),
            "snapshots": {"face_crop": self._event_asset(assets["face_crop"])},
        })
        self.last_emitted[candidate["track_id"]] = now
        self.last_embeddings[candidate["track_id"]] = embedding.copy()
        self.metrics.emitted()

    def _save_candidate_assets(self, candidate, sample_id: str) -> dict[str, dict[str, Any]]:
        ...
        frame_digest, frame_size = _write_jpeg_bytes(frame_path, candidate["event_frame_jpeg"])
        crop_digest, crop_size = _write_jpeg_bytes(crop_path, candidate["face_crop_jpeg"])
        return {
