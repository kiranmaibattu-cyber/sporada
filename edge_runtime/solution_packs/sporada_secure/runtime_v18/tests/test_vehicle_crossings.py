...
def test_uploader_removes_record_only_after_matching_ack(tmp_path):
    ...
    metrics = SimpleNamespace(acknowledged=lambda: None, failed=lambda: None, outbox=lambda count: None)
    uploader = VehicleCrossingUploader(outbox, "http://management/vehicle-crossings", "token", metrics)
...
def test_non_retryable_http_error_blocks_record(tmp_path):
    outbox = tmp_path / "outbox"
    outbox.mkdir()
    snapshot = tmp_path / "crossing.jpg"
    ...
    record = {
        "event": {"event_id": "event-1"},
        "snapshot": {"path": str(snapshot), "sha256": digest, "size_bytes": snapshot.stat().st_size},
        "delivery": {"attempts": 0},
    }
    record_path = outbox / "event-1.json"
    record_path.write_text(json.dumps(record))
    failures = []
    metrics = SimpleNamespace(acknowledged=lambda: None, failed=lambda: failures.append(True), outbox=lambda count: None)
    uploader = VehicleCrossingUploader(outbox, "http://management/vehicle-crossings", "token", metrics)
    uploader.opener = SimpleNamespace(open=lambda *args, **kwargs: (_ for _ in ()).throw(
        HTTPError("http://management", 400, "bad request", {}, None)
    ))

    uploader._submit(record_path, record)

    persisted = json.loads(record_path.read_text())
    assert persisted["delivery"]["blocked"] is True
    assert failures == [True]


def test_retryable_http_error_keeps_record_with_jittered_backoff(tmp_path):
    outbox = tmp_path / "outbox"
    outbox.mkdir()
    snapshot = tmp_path / "crossing.jpg"
    snapshot.write_bytes(b"jpeg-evidence")
    ...
    uploader = VehicleCrossingUploader(outbox, "http://management/vehicle-crossings", "token", metrics)
    uploader.opener = SimpleNamespace(open=lambda *args, **kwargs: (_ for _ in ()).throw(
        HTTPError("http://management", 503, "unavailable", {}, None)
    ))

    uploader._submit(record_path, record)

    persisted = json.loads(record_path.read_text())
    assert persisted["delivery"]["blocked"] is False
    assert persisted["delivery"]["attempts"] == 1
    assert persisted["delivery"]["next_attempt_at"] > persisted["delivery"]["last_attempt_at"]


def test_oversized_snapshot_is_blocked_before_network_submission(tmp_path, monkeypatch):
    outbox = tmp_path / "outbox"
    outbox.mkdir()
    snapshot = tmp_path / "crossing.jpg"
    snapshot.write_bytes(b"x")
    monkeypatch.setattr("services.worker.pipeline.vehicle_crossings.MAX_SNAPSHOT_BYTES", 0)
    digest = hashlib.sha256(snapshot.read_bytes()).hexdigest()
    record = {
        "event": {"event_id": "event-1"},
        "snapshot": {"path": str(snapshot), "sha256": digest, "size_bytes": snapshot.stat().st_size},
    ...
    persisted = json.loads(record_path.read_text())
    assert persisted["delivery"]["blocked"] is True
    assert "maximumBytes" in persisted["delivery"]["last_error"]
    assert failures == [True]
