...
import pipeline.face_samples as face_samples_module  # noqa: E402
...
def test_best_usable_sample_is_emitted_when_selection_window_expires(monkeypatch, tmp_path):
    class ImprovingExtractor(FakeExtractor):
        def __init__(self):
            self.qualities = iter((0.3, 0.6))
    ...
    clock = {"now": 100.0}
    monkeypatch.setattr(face_samples_module.time, "time", lambda: clock["now"])
    state = tmp_path / "state"
    monkeypatch.setenv("APEXFABRIC_STATE_ROOT", str(state))
    monkeypatch.setenv("SNAPSHOT_ROOT", str(state / "snapshots"))
    monkeypatch.setenv("FACE_PROCESS_INTERVAL", "1")
    ...
    config = {"analytics": {"face_recognition": {
        ...
        "emission": {
            "minimum_quality": 0.2, "preferred_quality": 0.8,
            "selection_window_seconds": 1.0, "cooldown_seconds": 5,
            "material_change_threshold": 0.15,
        },
    }}, "runtime_analytics": {"face_recognition": {"zones": []}}}
    pipeline = FaceSamplePipeline("cam1", "edge1", ImprovingExtractor(), config)
    person = SimpleNamespace(model_name="vehicle", class_name="pedestrian",
                             bbox=[10, 10, 100, 115], metadata={"track_id": 7})

    first = FramePacket(index=1, name="cam1", frame=np.full((120, 160, 3), 20, dtype=np.uint8))
    pipeline.process(first, [person])
    assert not first.analytics_events
    assert not list((state / "face_samples/outbox/cam1").glob("*.json"))

    clock["now"] = 100.4
    better = FramePacket(index=2, name="cam1", frame=np.full((120, 160, 3), 180, dtype=np.uint8))
    pipeline.process(better, [person])
    assert not better.analytics_events

    clock["now"] = 101.1
    expiry = FramePacket(index=3, name="cam1", frame=np.zeros((120, 160, 3), dtype=np.uint8))
    pipeline.process(expiry, [])

    records = list((state / "face_samples/outbox/cam1").glob("*.json"))
    assert len(records) == 1
    record = json.loads(records[0].read_text())
    assert record["sample"]["quality"] == 0.6
    assert len(expiry.analytics_events) == 1
    crop = cv2.imread(record["artifact"]["path"])
    assert crop is not None and float(crop.mean()) > 170
