...
import traffic_pilot_runtime.solution_image_entrypoint as entrypoint
...
def test_crossing_is_explicitly_suppressed_from_sse_and_counted():
    before = entrypoint._vehicle_crossing_sse_suppressed
    line = json.dumps({
        "camera_id": "cam1",
        "events": [{
            "event_id": "crossing-1",
            "event_type": "vehicle_entry_exit_crossed",
            "application": "vehicle_entry_exit_counts",
        }],
    })

    assert _events_from_jsonl_line(line) == []
    assert entrypoint._vehicle_crossing_sse_suppressed == before + 1


def test_crossing_delivery_metrics_are_exported(monkeypatch, tmp_path):
    state_root = tmp_path / "state"
    metrics_root = state_root / "metrics"
    metrics_root.mkdir(parents=True)
    (metrics_root / "vehicle-crossing-cam1.json").write_text(json.dumps({
        "camera_id": "cam1",
        "outbox_records": 2,
        "acknowledgements": 3,
        "submission_failures": 4,
    }))
    monkeypatch.setenv("APEXFABRIC_STATE_ROOT", str(state_root))

    payload = _metrics(RuntimeState("/configs/desired_state.json"))

    assert 'vehicle_crossing_outbox_records{camera_id="cam1"} 2' in payload
    assert 'vehicle_crossing_acknowledgements_total{camera_id="cam1"} 3' in payload
    assert 'vehicle_crossing_submission_failures_total{camera_id="cam1"} 4' in payload
    assert "vehicle_crossing_sse_suppressed_total" in payload
