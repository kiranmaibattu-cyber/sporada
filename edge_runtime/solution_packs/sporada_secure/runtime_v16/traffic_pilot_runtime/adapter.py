...
def _use_case_config(app: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    lines = _line_items_for(config, app)
    result = {
        "enabled": True,
        "lines": [_line_geometry(item) for item in lines],
...
def _line_items_for(config: dict[str, Any], app: str) -> list[dict[str, Any]]:
    counting_lines = config.get("counting_lines") or {}
    if not isinstance(counting_lines, dict):
        return []
    items = counting_lines.get(app) or []
    return items if isinstance(items, list) else []


def _line_geometry(item: dict[str, Any]) -> dict[str, Any]:
    return {
        "id": item["id"],
        "name": item["name"],
        "shape": "line",
        "points": _point_dicts([item["a"], item["b"]]),
        "purpose": "vehicle_entry_exit_counts",
        "normalized": True,
        "direction_mapping": dict(item["direction_mapping"]),
        "vehicle_classes": list(item["vehicle_classes"]),
        "minimum_track_age_frames": int(item.get("minimum_track_age_frames", 3)),
        "minimum_crossing_displacement": float(item.get("minimum_crossing_displacement", 0.02)),
        "crossing_cooldown_seconds": float(item.get("crossing_cooldown_seconds", 10)),
    }
