...
class VehicleCrossingUploader(threading.Thread):
    ...
    def __init__(self, outbox: Path, endpoint: str, token: str) -> None:
    ...
    def _submit(self, record_path: Path, record: dict[str, Any]) -> None:
        ...
        if digest != snapshot["sha256"] or len(image) != snapshot["size_bytes"]:
            raise ValueError("vehicle crossing evidence changed after persistence")
    ...
    @staticmethod
    def _defer(path: Path, record: dict[str, Any], exc: Exception) -> None:
        ...
        delivery.update({
            ...
            "next_attempt_at": time.time() + min(60.0, 2 ** min(attempts, 6)),
        })
...
class VehicleCrossingPipeline:
    ...
    def process(self, packet) -> None:
        ...
        for detection in packet.detections:
            ...
            for line in lines:
                if detection.class_name not in set(line.get("vehicle_classes") or []):
                    continue
