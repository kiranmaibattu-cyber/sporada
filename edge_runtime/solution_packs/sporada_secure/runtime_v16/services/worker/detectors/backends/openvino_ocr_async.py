...
def decode_logits(logits: np.ndarray) -> tuple[str, float]:
    """Decode CCT output and return geometric-mean character confidence."""
    scores = np.asarray(logits, dtype=np.float64)
    if scores.ndim == 3:
        scores = scores[0]
    if scores.ndim != 2 or scores.shape[-1] != len(ALPHABET):
        raise ValueError(f"unexpected OCR output shape: {scores.shape}")
    row_sums = scores.sum(axis=-1)
    if np.all(scores >= 0.0) and np.allclose(row_sums, 1.0, rtol=1e-3, atol=1e-4):
    ...
    keep = indices != ALPHABET.index(PAD)
    text = "".join(ALPHABET[index] for index in indices[keep])
    if not np.any(keep):
        return text, 0.0
    selected = probabilities[np.arange(len(indices))[keep], indices[keep]]
    confidence = float(np.exp(np.mean(np.log(np.clip(selected, 1e-12, 1.0)))))
    return text, confidence


class AsyncOCR:
    def __init__(self, model_path: str, device: str = "MULTI:GPU,NPU",
                 on_result: Callable[[str, float, Tuple], None] = None,
    ...
    def _on_done(self, request, userdata: Tuple) -> None:
        try:
            ...
            if self.on_result is not None:
                self.on_result(text, confidence, userdata)
        except Exception:  # noqa: BLE001
