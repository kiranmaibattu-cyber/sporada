...
SSE_CONTRACT_APPLICATIONS = frozenset({
    "anpr",
    "vehicle_counting",
    "pedestrian_counting",
...
DURABLE_ONLY_EVENT_TYPES = frozenset({"vehicle_entry_exit_crossed"})
_crossing_metric_lock = threading.Lock()
_vehicle_crossing_sse_suppressed = 0
IDENTITY_MAP = {
...
def _metrics(state: RuntimeState) -> str:
    ...
    for path in sorted(metrics_root.glob("vehicle-crossing-*.json")) if metrics_root.exists() else ():
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
            camera = _prometheus_label(str(payload["camera_id"]))
        except (OSError, ValueError, KeyError, json.JSONDecodeError):
            continue
        lines.append(
            f'vehicle_crossing_outbox_records{{camera_id="{camera}"}} '
            f'{int(payload.get("outbox_records", 0))}'
        )
        lines.append(
            f'vehicle_crossing_acknowledgements_total{{camera_id="{camera}"}} '
            f'{int(payload.get("acknowledgements", 0))}'
        )
        lines.append(
            f'vehicle_crossing_submission_failures_total{{camera_id="{camera}"}} '
            f'{int(payload.get("submission_failures", 0))}'
        )
    with _crossing_metric_lock:
        suppressed = _vehicle_crossing_sse_suppressed
    lines.append(f"vehicle_crossing_sse_suppressed_total {suppressed}")
    return "\n".join(lines) + "\n"
...
def _normalize_worker_event(envelope: dict[str, Any], event: dict[str, Any]) -> dict[str, Any] | None:
    global _vehicle_crossing_sse_suppressed
    ...
    if raw_event_type in DURABLE_ONLY_EVENT_TYPES:
        with _crossing_metric_lock:
            _vehicle_crossing_sse_suppressed += 1
        return None
    identity = IDENTITY_MAP.get(raw_application) or IDENTITY_MAP.get(raw_event_type)
    if not identity:
        return None
    application, normalized_type = identity
    if application not in SSE_CONTRACT_APPLICATIONS or normalized_type not in CONTRACT_EVENT_TYPES:
        return None
    payload = dict(event)
