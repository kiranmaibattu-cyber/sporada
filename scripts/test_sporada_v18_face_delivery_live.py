"""Exercise v18 best-sample selection and artifact-first delivery."""
        ...
        "cameras": [{
            ...
            "config": {
                ...
                "emission": {"minimum_quality": 0.2, "preferred_quality": 0.65,
                             "selection_window_seconds": 1.5, "cooldown_seconds": 5,
...
def main() -> None:
    ...
    try:
        ...
        wait_until(ready, "v18 image did not become ready")
        ...
        face_event = wait_until(
            lambda: next((event for event in sse_events(sse_path)
                          if event.get("application") == "face_recognition"
                          and (event.get("payload") or {}).get("sample_id") == sample_id), None),
            "no matching face_seen event reached the SSE client",
        )
        wait_until(
        ...
        wait_until(ready, "v18 image did not recover after restart")
        with receiver.lock:
            receiver.allow_samples = True
        sample = wait_until(lambda: receiver.samples.get(sample_id), "pending embedding was not resumed")

        schema = json.loads((ROOT / "delivery/apexfabric-v1/intel-285h/sporada-secure-v18/face-sample.schema.json").read_text())
        jsonschema.validate(sample, schema)
        event_schema = json.loads((ROOT / "delivery/apexfabric-v1/intel-285h/sporada-secure-v18/analytics-event.schema.json").read_text())
        ...
        crop = cv2.imread(str(local_crop))
