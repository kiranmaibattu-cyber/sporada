"""Exercise the v16 artifact-first face transaction across a container restart."""
...
RUN = ROOT / "run" / f"sporada-v16-face-delivery-{int(time.time())}"
NAME = "sporada-v16-face-delivery"
IMAGE = "localhost/sporada:intel-285h-2026.09.22-v16"
API = "http://127.0.0.1:18080"
TOKEN = "v16-live-test-token"
...
def main() -> None:
    ...
    try:
        ...
        wait_until(ready, "v16 image did not become ready")
        ...
        wait_until(ready, "v16 image did not recover after restart")
        with receiver.lock:
            receiver.allow_samples = True
        sample = wait_until(lambda: receiver.samples.get(sample_id), "pending embedding was not resumed")

        schema = json.loads((ROOT / "delivery/apexfabric-v1/intel-285h/sporada-secure-v16/face-sample.schema.json").read_text())
