...
BASE_V1_TAG="${SPORADA_V17_BASE_V1_TAG:-localhost/sporada-intel-runtime-base:intel-285h-2026.08.24-v1}"
BASE_TAG="${SPORADA_V17_BASE_TAG:-localhost/sporada-intel-runtime-base:intel-285h-2026.09.18-v2}"
IMAGE_VERSION="${APEXFABRIC_IMAGE_VERSION:-2026.09.22-v17}"
IMAGE_TAG="localhost/sporada:intel-285h-${IMAGE_VERSION}"

if [[ "${BUILD_SPORADA_V17_BASE:-0}" == "1" ]] || ! podman image inspect "$BASE_TAG" >/dev/null 2>&1; then
...
if ! podman image inspect "$BASE_TAG" >/dev/null 2>&1; then
  ...
  echo "Build the Intel base with BUILD_SPORADA_V17_BASE=1." >&2
  exit 1
fi
...
podman build --platform linux/amd64 \
  ...
  -f docker/Dockerfile.sporada-v17 \
  -t "$IMAGE_TAG" .
